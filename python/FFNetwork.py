...
class Network():
    ...
    def RandomWeights(self, mult=1.):
        for idx,w in enumerate(self.W):
            m = self.lyr[idx+1].N
            n = self.lyr[idx].N
            self.W[idx] = np.random.normal(size=[n,m])/np.sqrt(n)*mult
    ...
    def BackProp(self, t, lrate=0.05, decay=0.):
        '''
            ...
            Inputs:
             t      an array of targets (number of samples must match the
                    network's output)
             lrate  learning rate
             decay  is the weight-decay coefficient
        '''
        ...
        for i in range(self.n_layers-2, -1, -1):
            ...
            self.W[i] -= lrate*dEdW + decay*self.W[i]
            #self.lyr[i+1].b -= lrate*dEdb + decay*self.lyr[i+1].b


    def SGD(self, inputs, targets, lrate=0.05, epochs=1, batch_size=10, decay=0., progress=10):
        '''
            progress = net.SGD(inputs, targets, lrate=0.05, epochs=1, batch_size=10, decay=0, progress=10)
            ...
            Inputs:
              ...
              decay   is the decay coefficient for the weights (try lrate/100)
              progress how many epochs between reporting progress
        ...
        for k in range(epochs):
            ...
            if np.mod(k, progress)==0 and k>0:
